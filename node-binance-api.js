...
let api = function Binance(options = {}) {
    ...
    const getProxy = () => {
        if (Binance.options.proxyList.length > 0) {
            const proxy = Binance.options.proxyList[currentProxyIndex];
            currentProxyIndex = (currentProxyIndex + 1) % Binance.options.proxyList.length;
            return `http://${proxy}`;
        }
        return process.env.http_proxy || false
    };
    ...
    return {
        ...
        */
        ...
        websockets: {
             ...
             * @param {string} speed - the websocket update speed
             * @return {string} the websocket endpoint
             */
            depthCache: function depthCacheFunction(symbols, callback, limit = 500, speed = '100ms') {
                let reconnect = () => {
                    if (Binance.options.reconnect) depthCacheFunction(symbols, callback, limit, speed);
                };
                ...
                if (Array.isArray(symbols)) {
                    ...
                    subscription = subscribe(symbol.toLowerCase() + `@depth@${speed}`, handleDepthStreamData, reconnect, function () {
